...
void setup() {
  ...
  Serial.println("<TtoS01: Hola lucero");
}
...
void loop() {
  ...
  switch (appData) {
    case CMD_SPECIAL:
      ...
      // Display the data once every 100 times special command are received.
      if(displayPrescaler++ >= 100){
        displayPrescaler = 0;
        for(int i = 0; i < 3; i++){
          Serial.println("<Text0" + String(i) + ":" + String(Accel[i]));  
        }
      }
    ...
    default:
      ...
      for (int i = 0; i < MAX_RELAYS; i++) {
        if (appData == CMD_ON[i]) {
          ...
          Serial.println("<TtoS01: pinche beaner no aprietes el boton");
          ...
          Serial.println("<TtoS01: foco apagado");
        }
      }
  }
