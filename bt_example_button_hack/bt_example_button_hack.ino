...
String RelayAppId[MAX_RELAYS] = {"04", "05", "06", "07", "08", "09", "10", "11"};
// Command list (turn on - off for eachr relay)
const char CMD_ON[MAX_RELAYS] = {'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L'};
const char CMD_OFF[MAX_RELAYS] = {'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l'};

// This is the hack
boolean mLatch[MAX_RELAYS];
...
void setup() {
  ...
  // Initialize Output PORTS and software latches
...
void loop() {
  ...
  default:
    ...
    for(int i = 0; i < MAX_RELAYS; i++){
      if(appData == CMD_OFF[i]){// If image button pressed
